...
public class ComputerInterface
{
    ...
    public static void getInput(String colour, Board current, boolean isFirstTime, int filledSquares, String computerColour) throws java.io.IOException {
        current.state  = "";
        if(isFirstTime == false && LegalMove.isLegalMovePossible(current, colour) == false){
            if(colour.equals("b")){
                if(LegalMove.isLegalMovePossible(current, "w") == false){
                    ...
                    getInput("w", current, isFirstTime, filledSquares, computerColour);
                }
            }
            else{
                if(LegalMove.isLegalMovePossible(current, "b") == false){
                    ...
                    getInput("b", current, isFirstTime, filledSquares, computerColour);
                }
            }
        }
        else if(filledSquares < 64){
            if(colour.equals(computerColour) == false){
                ...
                if(newBoard.state.equals("") && isEmpty == false && filledSquares < 64){
                    ...
                    if(colour.equals("b")){
                        colour = "w";
                        getInput(colour, newBoard, false, filledSquares + 1, computerColour);
                    }
                    else{
                        colour = "b";
                        getInput(colour, newBoard, false, filledSquares + 1, computerColour);
                    ...
                    current.pieces = old;
                    getInput(colour, current, isFirstTime, filledSquares, computerColour);
                }
            }
            else{
                System.out.println("Coming up with a move...");
                int move = Computer.getBestMove(current, computerColour);
                if(current.log == true){
                    Logger.write(Logger.getPath(), move + computerColour + "!");
                }
                Piece toAdd = new Piece(move/10, move%10, colour);
                current = getOutput(current, toAdd);
                Board.printBoard(current);
                if(computerColour.equals("w")){
                    colour = "b";
                }
                else{
                    colour = "w";
                }
                getInput(colour, current, isFirstTime, filledSquares + 1, computerColour);
            }
        }
        else{
            over(current);
    ...
    public static void init(boolean log) throws java.io.IOException {
        ...
        board.log = log;
        Scanner checkColour = new Scanner(System.in);
        System.out.println("Press \"b\" is you want to play with black, and any other key to play with white");
        String colour;
        if(checkColour.nextLine().equals("b")){
            colour = "w";
        }
        else{
            colour = "b";
        ...
        if(log == true){
            ...
            FileWriter fw = new FileWriter(Logger.getPath());    
            fw.write("Logging has begun...!");
            fw.write("Playing against computer.!");
            fw.write("Computer is playing as " + colour + ".!");
            fw.close();
        }
        getInput("b", board, true, 4, colour);
    }
}
